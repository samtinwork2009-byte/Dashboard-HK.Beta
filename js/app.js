...
async function loadNewsSummary() {
  ...
  async function parseRssFeed(url) {
    ...
    const items = Array.from(xml.querySelectorAll('item')).slice(0, 8).map(node => ({
  ...
  try {
    ...
    inner.innerHTML = items.map(item => {
      ...
      const pubTime = new Date(item.pubDate).toLocaleTimeString('zh-HK', { hour12:false, hour:'2-digit', minute:'2-digit' });
