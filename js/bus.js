...
const PRESET_STOPS = [
  ...
  { operator:'KMB', id:'',                 label:'屯門常用路線',   route:'66X', serviceType:1, hint:'往觀塘/九龍' },
  { operator:'KMB', id:'',                 label:'屯門常用路線',   route:'690', serviceType:1, hint:'往天水圍/元朗' },
  { operator:'KMB', id:'',                 label:'元朗常用路線',   route:'278', serviceType:1, hint:'往沙田/大埔' },
  { operator:'KMB', id:'',                 label:'天水圍常用路線', route:'263', serviceType:1, hint:'往九龍' },
];
...
function getBusOperatorFromSelection(targetPrefix = '') {
  const key = targetPrefix === 'transport' ? 'bus-operator-select-transport' : 'bus-operator-select';
  const el = document.getElementById(key);
  return (el?.value || 'AUTO').toUpperCase();
}

function getBusSearchTargets(operator) {
  if (operator === 'AUTO') return ['KMB', 'CTB', 'LWB', 'NLB', 'MTR'];
  return [operator];
}
...
async function getRouteStops(operator, route, direction) {
  if (operator === 'KMB') {
    ...
    try {
      const r = await fetch(`https://data.etabus.gov.hk/v1/transport/kmb/route-stop/${route}/${bound}/1`);
      const d = await r.json();
  ...
  if (operator === 'CTB') {
    const bound = direction === 'inbound' ? 'inbound' : 'outbound';
    try {
      const r = await fetch(`https://rt.data.gov.hk/v2/transport/citybus/route-stop/CTB/${route}/${bound}`);
      const d = await r.json();
      return Array.isArray(d.data) ? d.data : [];
    } catch { return []; }
  }
...
const Bus = (function() {
  ...
  function renderPresetGrid(targetId = 'bus-presets-grid', idPrefix = 'preset') {
  ...
  async function renderSummary(targetId = 'summary-transport') {
    ...
    try {
      const items = await Promise.all(PRESET_STOPS.slice(0, 5).map(async (p) => {
        let etas = [];
        if (p.operator === 'KMB') {
  ...
  async function refresh() {
    ...
    await Promise.allSettled(PRESET_STOPS.map((p, i) => Promise.all([
...
const BusSearch = (function() {
  ...
  async function search(targetPrefix = '') {
    const inputId = targetPrefix === 'transport' ? 'bus-search-input-transport' : 'bus-search-input';
    const inputEl = document.getElementById(inputId);
    const resultEl = document.getElementById(id('bus-search-result', targetPrefix));
    ...
    for (const operator of targets) {
      ...
      if (outboundStops.length) {
        results.push({ operator, route: input, direction: 'outbound', orig: routeInfo.orig, dest: routeInfo.dest, stops: outboundStops });
      }
      if (inboundStops.length) {
        results.push({ operator, route: input, direction: 'inbound', orig: routeInfo.dest, dest: routeInfo.orig, stops: inboundStops });
      }
      if (!outboundStops.length && !inboundStops.length && operator === selectedOperator) {
        results.push({ operator, route: input, direction: 'outbound', orig: routeInfo.orig, dest: routeInfo.dest, stops: [], note: getOperatorHint(operator) });
      }
    }
  ...
  function renderDirectionTabs(targetPrefix) {
    ...
    const tabsHtml = _results.map((r, i) => {
      ...
      const noteText = r.note ? `<div style="font-size:10px;opacity:.72">${r.note}</div>` : '';
      return `
    ...
    const transportResult = document.getElementById('bus-search-result-transport');
    if (transportResult && transportResult !== resultEl) {
      const copy = resultEl.innerHTML.replace(/id="[^"]+"/g, '').replace(/onclick="[^"]*"/g, '');
  ...
  async function renderStopList(idx, targetPrefix) {
    ...
    `;
    const transportResult2 = document.getElementById('bus-search-result-transport');
    const currentResultWrapper = document.getElementById(id('bus-search-result', targetPrefix));
    if (transportResult2 && currentResultWrapper && transportResult2 !== currentResultWrapper) {
      transportResult2.innerHTML = currentResultWrapper.innerHTML.replace(/id="[^"]+"/g,'').replace(/onclick="[^"]*"/g,'');
    }
  ...
  function _renderStopButtons(listEl, stops, nameMap, r, orig, dest, targetPrefix) {
    ...
    const buttonsHtml = stops.map((s, i) => {
      ...
      const safeName = name ? name.replace(/"/g, '&quot;') : '';
        ...
        const clickAttr = name
          ? 'data-stop="' + s.stop + '" data-op="' + r.operator + '" data-route="' + r.route + '" data-dir="' + r.direction + '" data-name="' + safeName + '" data-panel="' + panelId + '"'
        : 'disabled';
    ...
    const transportResult2 = document.getElementById('bus-search-result-transport');
    const sourceResult     = document.getElementById(id('bus-search-result', targetPrefix));
    if (transportResult2 && sourceResult && transportResult2 !== sourceResult) {
      transportResult2.innerHTML = sourceResult.innerHTML.replace(/id="[^"]+"/g,'').replace(/onclick="[^"]*"/g,'');
    }
  }
  ...
  async function selectDir(idx, targetPrefix) {
    ...
    _results.forEach((_, i) => {
      const btn = document.getElementById(`dir-tab-${i}${targetPrefix?'-'+targetPrefix:''}`);
      if (!btn) return;
      const active = i === idx;
  ...
  /* ── Load ETA for stop ───────────────────────────────── */
  async function loadETA(stopId, operator, route, serviceType, stopName, direction, panelId) {
  ...
  async function _fetchETA(stopId, operator, route, serviceType, stopName, direction, panel) {
    try {
      ...
      if (operator === 'KMB') {
        ...
        panel.innerHTML = headerHtml + `<div style="padding:16px;text-align:center;color:var(--text-faint);font-size:13px">${getOperatorHint(operator) || '此營辦商暫無實時到站資料。'}</div>`;
        return;
      }
  ...
  function _refresh(stopId, operator, route, serviceType, stopName, direction, panelId) {
    const panel = document.getElementById(panelId || 'bus-eta-panel');
    if (!panel) return;
...
const Bus_GMB = (function() {
  const GMB_CATALOG = {
    NT: [
      { route_code:'44', description:'屯門 ↔ 元朗', directions:[{ orig_tc:'屯門', dest_tc:'元朗', stops:[{stop_id:'TM1',name_tc:'屯門市中心'},{stop_id:'TM2',name_tc:'兆康苑'},{stop_id:'YL1',name_tc:'元朗站'}] }] },
      { route_code:'44A', description:'屯門 ↔ 天水圍', directions:[{ orig_tc:'屯門', dest_tc:'天水圍', stops:[{stop_id:'TM3',name_tc:'屯門市中心'},{stop_id:'TSW1',name_tc:'天瑞邨'},{stop_id:'TSW2',name_tc:'天水圍站'}] }] },
      { route_code:'44B', description:'屯門 ↔ 洪水橋', directions:[{ orig_tc:'屯門', dest_tc:'洪水橋', stops:[{stop_id:'TM4',name_tc:'屯門市中心'},{stop_id:'HS1',name_tc:'洪水橋'},{stop_id:'HS2',name_tc:'新田'}] }] },
      { route_code:'44C', description:'屯門 ↔ 元朗南', directions:[{ orig_tc:'屯門', dest_tc:'元朗南', stops:[{stop_id:'TM5',name_tc:'屯門市中心'},{stop_id:'YL2',name_tc:'青山公路'},{stop_id:'YL3',name_tc:'元朗南'}] }] },
    ],
    KLN: [
      { route_code:'44', description:'觀塘 ↔ 九龍灣', directions:[{ orig_tc:'觀塘', dest_tc:'九龍灣', stops:[{stop_id:'KT1',name_tc:'觀塘站'},{stop_id:'KLB1',name_tc:'九龍灣站'},{stop_id:'KLB2',name_tc:'彩虹'}] }] },
      { route_code:'44A', description:'荔景 ↔ 深水埗', directions:[{ orig_tc:'荔景', dest_tc:'深水埗', stops:[{stop_id:'LK1',name_tc:'荔景站'},{stop_id:'SP1',name_tc:'深水埗站'},{stop_id:'SP2',name_tc:'長沙灣'}] }] },
      { route_code:'44B', description:'新蒲崗 ↔ 九龍城', directions:[{ orig_tc:'新蒲崗', dest_tc:'九龍城', stops:[{stop_id:'SPK1',name_tc:'新蒲崗'},{stop_id:'KL1',name_tc:'九龍城'},{stop_id:'KL2',name_tc:'啟德'}] }] },
    ],
    HKI: [
      { route_code:'44', description:'港島東區 ↔ 中環', directions:[{ orig_tc:'港島東區', dest_tc:'中環', stops:[{stop_id:'HK1',name_tc:'港島東'},{stop_id:'HK2',name_tc:'銅鑼灣'},{stop_id:'HK3',name_tc:'中環'}] }] },
      { route_code:'44A', description:'柴灣 ↔ 灣仔', directions:[{ orig_tc:'柴灣', dest_tc:'灣仔', stops:[{stop_id:'CW1',name_tc:'柴灣'},{stop_id:'WA1',name_tc:'灣仔'},{stop_id:'WA2',name_tc:'銅鑼灣'}] }] },
    ],
  };
  ...
  async function loadRoutes(targetPrefix = '') {
    const region = document.getElementById(targetPrefix ? `gmb-region-${targetPrefix}` : 'gmb-region')?.value || 'NT';
  ...
  function getRouteMeta(region, routeId) {
    return getCatalog(region).find(r => String(r.route_code).toUpperCase() === String(routeId).toUpperCase()) || getCatalog(region)[0];
  }
...
document.addEventListener('click', function(e) {
  ...
  if (kmbBtn) {
    ...
    if (stopId && op && route) BusSearch.loadETA(stopId, op, route, '1', name, dir, kmbBtn.dataset.panel);
    return;
  }
  // GMB stop buttons
