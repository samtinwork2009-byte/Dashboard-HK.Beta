...
window.getDashboardFallbackText = function () {
	return "此項目有待更新，敬請原諒";
};
...
(function initTheme() {
	...
	let initial =
		getHashTheme() || getStoredTheme() || getSystemTheme() || "dark";
	...
	if (btn) {
		btn.addEventListener("click", function () {
			...
			window.localStorage.setItem("hkdb-theme", next);
	...
	function getStoredTheme() {
		try {
			let stored = window.localStorage.getItem("hkdb-theme");
	...
	window.addEventListener("hashchange", function () {
		let t = getHashTheme();
		if (t && t !== document.documentElement.dataset.theme) {
...
async function loadLunarDate() {
	...
	try {
		...
		console.error("Failed to load lunar date:", e);
...
document.addEventListener("click", function (event) {
	...
	if (
		!event.target.closest(".more-menu-panel") &&
		!event.target.closest('[onclick*="toggleMoreMenu"]')
	) {
...
window.aqhiClass = function (val) {
	...
	if (Number.isNaN(n)) return "tag-muted";
...
/* ── Simple top notification / toast ── */
window.showNotification = function (text, opts = { duration: 6000 }) {
	try {
		let el = document.getElementById("hk-notification");
		if (!el) {
			el = document.createElement("div");
			el.id = "hk-notification";
			el.style.cssText =
				"position:fixed;top:var(--nav-h);left:50%;transform:translateX(-50%);z-index:9999;padding:12px 18px;border-radius:10px;background:linear-gradient(90deg,rgba(0,112,192,0.9),rgba(167,139,250,0.9));color:white;box-shadow:0 8px 30px rgba(0,0,0,0.5);font-weight:700;display:none;";
			document.body.appendChild(el);
		}
		el.textContent = text;
		el.style.display = "block";
		clearTimeout(window._hk_notif_to);
		window._hk_notif_to = setTimeout(() => {
...
window.aedClass = function (mins) {
	...
	if (Number.isNaN(n) || mins === "" || mins === null) return "tag-muted";
...
function initPullToRefresh() {
	...
	function getIndicator() {
		...
		indicator.innerHTML =
			'<span class="ptr-arrow">\u2193</span><span class="ptr-text">\u4e0b\u62c9\u4ee5\u66f4\u65b0</span>';
	...
	document.addEventListener(
		...
		function (e) {
			if (!pulling) return;
			const dy =
				(e.changedTouches[0] ? e.changedTouches[0].clientY : startY) - startY;
			pulling = false;
