...
window.DASHBOARD_FALLBACK_TEXT = '此項目有待更新，敬請原諒';
window.getDashboardFallbackText = function() {
  return window.DASHBOARD_FALLBACK_TEXT;
};
...
(function initTheme() {
  ...
  var initial = getHashTheme() || getStoredTheme() || getSystemTheme() || 'dark';
  ...
  if (btn) {
    btn.addEventListener('click', function() {
      ...
      try { window.localStorage.setItem('hkdb-theme', next); } catch (e) {}
  ...
  function getStoredTheme() {
    try {
      var stored = window.localStorage.getItem('hkdb-theme');
...
window.showNotification = function(text, opts={duration:6000}){
  try{
    ...
    if (!el) {
      el = document.createElement('div'); el.id = 'hk-notification';
      el.style.cssText = 'position:fixed;top:var(--nav-h);left:50%;transform:translateX(-50%);z-index:9999;padding:12px 18px;border-radius:10px;background:linear-gradient(90deg,rgba(0,112,192,0.9),rgba(167,139,250,0.9));color:white;box-shadow:0 8px 30px rgba(0,0,0,0.5);font-weight:700;display:none;';
      document.body.appendChild(el);
    }
    el.textContent = text;
    el.style.display = 'block';
    clearTimeout(window._hk_notif_to);
    window._hk_notif_to = setTimeout(()=>{ el.style.display='none'; }, opts.duration||6000);
