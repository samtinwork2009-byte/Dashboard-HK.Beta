...
window.WeatherMap = (function(){
  ...
  async function fetchRHR(){
    ...
    }catch(e){console.warn('map fetchRHR failed',e);return null}
  }
  ...
  async function onMarkerClick(e, station){
    ...
    setTimeout(async ()=>{
      ...
      try{
        ...
        const s = temps.find(t=> (t.place||'').includes(station.id) || (t.place_ch||'').includes(station.id));
      ...
      try{
        new Chart(canvas.getContext('2d'), {
          type:'line', data:{ labels:data.map((_,i)=>''), datasets:[{ data:data, borderColor:'rgba(59,130,246,1)', tension:0.3, fill:true, backgroundColor:'rgba(59,130,246,0.08)'}] }, options:{ responsive:false, maintainAspectRatio:false, plugins:{legend:{display:false}}, scales:{x:{display:false}, y:{ display:true, ticks:{maxTicksLimit:4}}}}
        });
      }catch(err){console.warn('chart popup error', err)}
    }, 80);
  }

  function initMap(){
    const el = document.getElementById('w-map');
    if (!el) { console.warn('w-map element not found'); return; }
    map = L.map(el).setView(MAP_CENTER, MAP_ZOOM);
    L.tileLayer(MAP_TILE, { attribution: '&copy; OpenStreetMap contributors' }).addTo(map);

    STATIONS.forEach(st=>{
      const marker = L.circleMarker([st.lat, st.lon], { radius:8, color:'#2563eb', fillColor:'#60a5fa', fillOpacity:0.9 }).addTo(map);
      marker.bindPopup(makePopupHtml(st.id), { maxWidth:240 });
      marker.on('click', (e)=> { onMarkerClick(e, st); if (window.StationPanel && StationPanel.open) StationPanel.open(st.id); });
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  if (typeof L === 'undefined'){
    ...
    s.onload = ()=>{ try{ WeatherMap.initMap(); }catch(e){console.error(e)} };
    document.body.appendChild(s);
  } else { try{ WeatherMap.initMap(); }catch(e){console.error(e)} }
});
/* ============================================================
