...
window.Nowcast = (function(){
  ...
  async function fetchRHR(){
    try{
      ...
      return await res.json();
    }catch(e){console.warn('fetchRHR fallback failed',e);return null}
  }
  ...
  function renderFallback(labels, values){
    ...
    if (!ncChart){
      ncChart = new Chart(ctx, {
        type:'bar',
        data:{ labels: labels, datasets:[{ label:'降雨預測 (mm)', data: values, backgroundColor: 'rgba(59,130,246,0.9)' }]},
        options:{ responsive:true, maintainAspectRatio:false, scales:{ y:{ beginAtZero:true } } }
      });
    } else {
      ncChart.data.labels = labels; ncChart.data.datasets[0].data = values; ncChart.update();
    }
  }
  ...
  async function render2H(){
    ...
    if (two && two.data && two.data.length){
      ...
      try{
        ...
        if (first.forecast && first.forecast.length){
          ...
          for(let i=0;i<Math.min(first.forecast.length,5);i++){
            const v = Number(first.forecast[i].rain || first.forecast[i].precip || first.forecast[i].chance || 0) || 0;
            values[i] = v;
          }
      ...
      }catch(e){ console.warn('parse 2h structure', e); }
    ...
    let currentRain = 0;
    try{ currentRain = rhr?.rainfall?.data?.reduce((acc,s)=>acc + (Number(s.value)||0),0) / Math.max(1, (rhr?.rainfall?.data||[]).length); }catch(e){currentRain=0}
    const values = labels.map((_,i)=> Math.round(currentRain * (1 + (i*0.05)) * 10)/10 );
    renderFallback(labels, values);
  }
...
document.addEventListener('DOMContentLoaded', function(){
  if (typeof Chart !== 'undefined') { try{ Nowcast.render2H(); } catch(e){ console.error(e); } }
});
