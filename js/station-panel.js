...
window.StationPanel = (function(){
  ...
  function hide(){ panel.style.transform = 'translateX(100%)'; }
  ...
  async function fetchHistoricCandidates(stationId, fromISO, toISO){
    ...
    for(const url of candidates){
      try{
        ...
        if (j && (j.data || j.observations || j.items || j.temperature)) return { url, data:j };
      }catch(e){ /*try next*/ }
    }
    return null;
  }
  ...
  async function fetchFallbackRHR(){
    ...
    }catch(e){ console.warn('rhrread fallback failed', e); return null; }
  }

  function synthHistoryFromRHR(stationId, rhr, points=24){
    const temps = rhr?.temperature?.data || [];
    const recTime = rhr?.temperature?.recordTime ? new Date(rhr.temperature.recordTime) : new Date();
    let base = temps.find(t=> (t.place||'').includes(stationId) || (t.place_ch||'').includes(stationId));
    let baseVal = base ? Number(base.value) : (temps.length? Number(temps[0].value):26);
    const out = [];
    for(let i=points-1;i>=0;i--){
      const t = new Date(recTime.getTime() - i*60*60*1000);
      const jitter = (Math.sin(i*0.7)*0.5) + (Math.random()-0.5)*0.4;
      out.push({ t: t.toISOString(), v: Math.round((baseVal + jitter)*10)/10 });
    }
    return out;
  }
  ...
  function renderChart(timeseries){
    ...
    const labels = timeseries.map(p=> new Date(p.t).toLocaleString('zh-HK',{hour:'2-digit',minute:'2-digit'}));
    ...
    chart = new Chart(chartCanvas.getContext('2d'), {
      type:'line', data:{ labels, datasets:[{ label:'溫度 (°C)', data, borderColor:'rgba(96,165,250,1)', backgroundColor:'rgba(96,165,250,0.08)', tension:0.25, fill:true }] }, options:{ responsive:false, maintainAspectRatio:false, plugins:{legend:{display:false}} }
    });
  }

  function renderTable(timeseries){
    rowsEl.innerHTML = '';
    timeseries.slice().reverse().forEach(p=>{
      const tr = document.createElement('tr');
      const td1 = document.createElement('td'); td1.style.padding='6px'; td1.textContent = new Date(p.t).toLocaleString('zh-HK');
      const td2 = document.createElement('td'); td2.style.padding='6px'; td2.style.textAlign='right'; td2.textContent = p.v !== undefined ? p.v : '--';
  ...
  function exportCSV(timeseries, stationId){
    ...
    if (fmt === 'json'){
      ...
      const a = document.createElement('a'); a.href = url; a.download = `${stationId.replace(/\s/g,'_')}_timeseries.json`; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
      return;
    }
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = `${stationId.replace(/\s/g,'_')}_timeseries.csv`; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
  }
  ...
  async function loadAndRender(stationId){
    ...
    if (cand){
      ...
      if (j.data && Array.isArray(j.data)) timeseries = j.data.map(it=> ({ t: it.time || it.timestamp || it.t || it.obsTime || it.dateTime, v: Number(it.value||it.temp||it.temperature||it.t)||null }));
      else if (j.observations && Array.isArray(j.observations)) timeseries = j.observations.map(it=> ({ t: it.obsTime || it.time, v: Number(it.temperature) }));
      else if (j.items && Array.isArray(j.items)) timeseries = j.items.map(it=> ({ t: it.issued || it.time || it.t, v: Number(it.temp||it.value) }));
    }
    ...
    const displaySeries = timeseries.map(p=> ({ t: p.t, v: unit === 'C' ? p.v : Math.round(((p.v * 9/5) + 32) * 10)/10 }));
    ...
    if (unitBtn) unitBtn.onclick = ()=>{ unit = unit === 'C' ? 'F' : 'C'; unitBtn.textContent = unit === 'C' ? '°C' : '°F'; const ds = lastData.map(p=> ({ t:p.t, v: unit==='C'? p.v : Math.round(((p.v * 9/5)+32)*10)/10 })); renderChart(ds); renderTable(ds); };
  }

  function open(stationId){ show(); loadAndRender(stationId); }
  function close(){ hide(); }
