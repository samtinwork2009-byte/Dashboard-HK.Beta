...
function getCrowdingBadge(mins) {
  if (mins === null || mins === undefined || mins === '--') return '<span class="tag tag-blue">班次資訊</span>';
  ...
  try {
    ...
    for (const dir of ['UP', 'DOWN']) {
      ...
      html += trains.slice(0, 4).map(function(t) {
        ...
        return '<div class="row-item" style="align-items:flex-start;gap:10px">' +
          '<span class="row-name">' + dest + platSpan + '<span style="display:block;font-size:10px;color:var(--text-faint);margin-top:4px">下一班約 ' + (mins !== '--' ? mins + ' 分' : '—') + '</span></span>' +
          '<span style="display:flex;flex-direction:column;align-items:flex-end;gap:6px"><span class="tag ' + cls + '">' + (mins !== '--' ? mins + ' 分鐘' : '—') + '</span>' + crowding + '</span>' +
          '</div>';
      }).join('');
      html += '</div>';
    }
  ...
  try {
    ...
    for (const plat of platforms) {
      ...
      html += routes.slice(0, 4).map(function(r) {
        ...
        return '<div class="row-item" style="align-items:flex-start;gap:10px">' +
          '<span class="row-name"><span style="font-weight:700;color:var(--primary);margin-right:6px">' + routeNo + '</span>' + dest + '<span style="display:block;font-size:10px;color:var(--text-faint);margin-top:4px">下一班 ' + timeCh + '</span></span>' +
          '<span style="display:flex;flex-direction:column;align-items:flex-end;gap:6px"><span class="tag ' + cls + '">' + timeCh + '</span>' + crowding + '</span>' +
          '</div>';
      }).join('');
      html += '</div>';
    }
