...
window.WeatherDetail = (function(){
  ...
  function render2HRain(rhr) {
    ...
    const top = rainArr.slice().sort((a,b)=> (b.value||0)-(a.value||0)).slice(0,6);
    top.forEach(s => {
    ...
    if (!rainChart) {
      rainChart = new Chart(ctx, {
        type: 'bar',
        data: { labels: labels, datasets: [{ label:'過去一小時降雨量 (mm)', data: buckets, backgroundColor:'rgba(96,165,250,0.9)'}] },
  ...
  function renderHourlyTemp(rhr) {
    ...
    if (!tempChart) {
      tempChart = new Chart(ctx, {
        type: 'line',
        data: { labels: labels, datasets: [{ label:'現在溫度 (°C)', data: values, borderColor:'rgba(125,211,252,1)', tension:0.3, fill:true, backgroundColor:'rgba(125,211,252,0.08)'}] },
  ...
  function renderStationCards(rhr){
    ...
    if (!container) return;
    const temps = rhr.temperature?.data || [];
    if (!temps.length){ container.innerHTML = '<div class="skel skel-p"></div>'; return; }
    // pick top 12 by temperature
    const sample = temps.slice().sort((a,b)=> (Number(b.value)||0)-(Number(a.value)||0)).slice(0,12);
    container.innerHTML = '';
    sample.forEach((s, idx)=>{
      ...
      if (canvas && typeof Chart !== 'undefined'){
        ...
        const ts = []; const labels = [];
        for(let i=7;i>=0;i--){ const t = new Date(Date.now() - i*3600*1000); labels.push(''); ts.push(Math.round((base + (Math.sin(i*0.6)*0.6) + (Math.random()-0.5)*0.4)*10)/10); }
        try{ new Chart(canvas.getContext('2d'), { type:'line', data:{ labels, datasets:[{ data:ts, borderColor:'rgba(125,211,252,1)', backgroundColor:'rgba(125,211,252,0.06)', tension:0.25, fill:true }] }, options:{ responsive:false, maintainAspectRatio:false, plugins:{legend:{display:false}}, scales:{x:{display:false}, y:{display:false}} } }); }catch(e){/* ignore */}
      }
    });
  }
  ...
  function renderRainGrid(rhr){
    ...
    if (!rain.length){ container.innerHTML = '<div class="skel skel-p"></div>'; return; }
    container.innerHTML = '';
    rain.slice().sort((a,b)=> (Number(b.max||b.value||0) - Number(a.max||a.value||0))).forEach(s=>{
      const place = s.place || s.place_ch || '區';
      const mm = Number(s.max ?? s.value ?? s.max) || 0;
      let cls = 'heat-low'; if (mm >= 10) cls='heat-high'; else if (mm >= 3) cls='heat-med';
      const tile = document.createElement('div'); tile.className = `heat-tile ${cls}`;
  ...
  async function renderAQHI(){
    ...
    if (window.Environment && typeof Environment.fetchAQHI === 'function'){
      try{
        ...
        const label = data?.label || data?.status || (idx ? `AQHI ${idx}` : 'AQHI');
        el.querySelector('div') && (el.querySelector('div').textContent = idx !== null ? idx : '--');
        if (desc) desc.textContent = data?.conclusion || data?.advice || label || '資料已更新';
        if (details) details.innerHTML = '';
        return;
      }catch(e){ console.warn('AQHI render failed', e); }
    }
  ...
  function renderSunTide(fnd, rhr){
    ...
    if (elSunrise) elSunrise.textContent = fnd?.sunrise || fnd?.weatherForecast?.[0]?.sunrise || elSunrise.textContent || '--';
    if (elSunset) elSunset.textContent = fnd?.sunset || fnd?.weatherForecast?.[0]?.sunset || elSunset.textContent || '--';
    // For tides, link to tide page (full tide data available in Tides section)
    const tideEl = document.getElementById('summary-tide');
    if (tideEl) tideEl.innerHTML = `<a href="#" onclick="showPage('tides')" style="color:var(--primary)">查看潮汐詳情</a>`;
  }
  ...
  const STATION_COORDS = [
    ...
    { id: '紅磡', lat:22.300, lon:114.182 },
    { id: '大埔', lat:22.450, lon:114.169 },
  ...
  async function locateAndApply() {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(async pos=>{
        ...
        if (locEl) locEl.textContent = `座標 ${lat.toFixed(4)}°N, ${lon.toFixed(4)}°E`;
        const stationName = pickNearestStation(lat,lon);
        if (!stationName) return;
        // fetch rhr to get station data
        const rhr = await fetchRHR();
        const temps = rhr?.temperature?.data || [];
        const s = temps.find(t=> (t.place||'').includes(stationName) || (t.place_ch||'').includes(stationName));
        if (s) {
          ...
          if (summaryTemp) summaryTemp.textContent = (s.value!==undefined? s.value : '--') + '°C';
          if (desc) desc.textContent = rhr?.weatherMain?.[0]?.text || rhr?.icon?.[0] || '';
          if (upd) upd.textContent = rhr?.temperature?.recordTime ? new Date(rhr.temperature.recordTime).toLocaleTimeString('zh-HK',{hour12:false}) : '';
        }

        // Also update HQ temp as reference
        const hq = temps.find(t => (t.place||'').includes('香港天文台') || (t.place||'').includes('天文台'));
        if (hq) {
          const htemp = document.getElementById('h-temp');
          if (htemp) htemp.textContent = hq.value !== undefined ? hq.value : '--';
        }
  ...
  function hideEmptyCards(){
    ...
    checks.forEach(([id,emptyText])=>{
      ...
      if (!txt || (emptyText && txt.includes(emptyText))) card.style.display='none'; else card.style.display='';
    });
  }
...
document.addEventListener('DOMContentLoaded', function(){
  if (typeof Chart !== 'undefined') {
    try { WeatherDetail.init(); } catch(e) { console.error(e); }
  } else {
