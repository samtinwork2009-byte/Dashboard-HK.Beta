...
function renderCurrentWeather(d) {
  ...
  const windText = windDir && windSpeed ? `${windDir} ${windSpeed} km/h` : '-- / --';
...
async function fetchAQHI() {
  if (typeof Environment !== 'undefined' && typeof Environment.fetchAQHI === 'function') {
...
function getDistrictFromCoords(lat, lng) {
  if (lng >= 113.95 && lng <= 114.35 && lat >= 22.25 && lat <= 22.45) return '屯門區';
  if (lng >= 113.85 && lng <= 114.25 && lat >= 22.35 && lat <= 22.55) return '元朗區';
  if (lng >= 114.05 && lng <= 114.25 && lat >= 22.30 && lat <= 22.40) return '沙田區';
  if (lng >= 114.15 && lng <= 114.25 && lat >= 22.30 && lat <= 22.35) return '大埔區';
  if (lng >= 114.10 && lng <= 114.25 && lat >= 22.30 && lat <= 22.40) return '西貢區';
  if (lng >= 114.10 && lng <= 114.20 && lat >= 22.25 && lat <= 22.35) return '葵青區';
  if (lng >= 114.15 && lng <= 114.20 && lat >= 22.30 && lat <= 22.35) return '荃灣區';
  if (lng >= 114.16 && lng <= 114.23 && lat >= 22.30 && lat <= 22.35) return '中西區';
  if (lng >= 114.15 && lng <= 114.20 && lat >= 22.30 && lat <= 22.35) return '九龍區';
  return '香港本地';
}
...
async function fetchLocationInfo() {
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(position => {
      ...
      district = getDistrictFromCoords(position.coords.latitude, position.coords.longitude) || '香港本地';
...
async function fetchTideSummary() {
  ...
  try {
    ...
    const high = hours.reduce((acc, cur) => cur.height > acc.height ? cur : acc, hours[0]);
    const low = hours.reduce((acc, cur) => cur.height < acc.height ? cur : acc, hours[0]);
...
async function fetchSunTimes() {
  ...
  try {
    const res = await fetch('https://api.sunrise-sunset.org/json?lat=22.3193&lng=114.1694&date=today&formatted=0');
    if (!res.ok) throw new Error(`SunTime HTTP ${res.status}`);
    const data = await res.json();
    if (data.status !== 'OK' || !data.results) throw new Error(`SunTime API ${data.status}`);

    const toLocal = iso => {
      const dt = new Date(iso);
      if (isNaN(dt.getTime())) return '--';
      return dt.toLocaleTimeString('zh-HK', { hour12: false, hour: '2-digit', minute: '2-digit' });
    };
...
function updateSummaryWeather(currentTemp, aqhi, desc, updatedAt, humidity, uv, condition, wind) {
...
async function fetchForecast() {
  try {
    ...
    if (summaryWeather) {
      updateSummaryForecast(`短期天氣：${summaryWeather.forecastWeather || summaryWeather.forecastWind || summaryWeather.week || ''}`);
    } else {
    ...
    if (localSummaryEl) {
      localSummaryEl.textContent = summaryWeather
        ? (summaryWeather.forecastWeather || summaryWeather.forecastWind || summaryWeather.week || d.generalSituation || '暫無預報')
...
function renderWarnsum(data) {
  ...
  const alertText = warnings.map(w => (w.name || w.type || '警告') + (w.code ? `(${w.code})` : '')).join(' · ');
  ...
  el.innerHTML = warnings.map(w => {
    ...
    let bannerClass = '';
    let emoji = '⚠️';
    if ((w.type || '').toUpperCase().includes('RAIN') || (w.code||'').toUpperCase().includes('RAIN')) {
      // WRAIN codes might be WAMB/WRED/WBLA etc.
      const codeUpper = (w.code||'').toUpperCase();
      if (codeUpper.includes('WRED') || codeUpper.includes('WRED')) { bannerClass = 'warn-red'; emoji='🌧️'; }
      else if (codeUpper.includes('WBLA') || codeUpper.includes('WAMB') ) { bannerClass = 'warn-yellow'; emoji='🌦️'; }
      else { bannerClass = 'warn-yellow'; emoji='🌧️'; }
    }
    ...
    // Save to history
    try { saveWarnHistory({ code: code||actionCode||name, name, issueTime, action: actionCode, tips }); } catch(e){}
...
function saveWarnHistory(entry) {
  ...
  } catch (e) { console.error('saveWarnHistory', e); }
}

function loadWarnHistory() {
  try { return JSON.parse(localStorage.getItem('hk_warn_history')||'[]'); } catch(e){ return []; }
}
  ...
  try {
    ...
    try {
      if (warningKeys.length) {
        ...
        if (window.showNotification) window.showNotification('天文台警報：' + label, { duration: 8000 });
      }
    } catch (e) {}
...
function getWarningMeta(data) {
  ...
  const primary = entries.find(e => e.type?.toUpperCase().includes('TY') || e.type?.toUpperCase().includes('TYPHOON')) ||
                  data['WTCSGNL'] || data['WRAIN'] || entries[0];
  ...
  meta.labelText = '現時生效警告：' + keys.map(k => {
    const entry = data[k];
  ...
  if (primaryKey === 'WFIRE' || primaryKey === 'WWIND' || primaryKey === 'WTMW' || primaryKey === 'WL') {
    meta.icon = '⚠️';
...
function renderWarningPopupContent(data) {
  ...
  const keys = Object.keys(data || {});
  if (!keys.length) {
    contentEl.innerHTML = '<div class="warning-popup-item"><div>目前沒有生效警告。</div></div>';
    updateEl.textContent = '最後更新：' + new Date().toLocaleString('zh-HK', { hour12: false });
    return;
  }
  ...
  contentEl.innerHTML = keys.map(key => {
    ...
    const status = action === 'ISSUE' ? '發布' : action === 'UPDATE' ? '更新' : '生效中';
    const guidance = getWarningGuidance(key, code);
    const details = [];
    if (entry.forecast || entry.details) details.push(entry.forecast || entry.details);
...
function getWarningGuidance(key, code) {
  ...
  if (upper.includes('WTCSGNL')) return '熱帶氣旋警告：固定鬆動物件，避免外出及海邊活動。';
  if (upper.includes('WRAIN')) return '暴雨警告：注意路面積水及山泥傾瀉，駕駛請減速慢行。';
  if (upper.includes('WFROST')) return '霜凍警告：戶外低溫，注意保暖及防滑。';
  if (upper.includes('WHOT')) return '酷熱警告：多喝水，避免長時間暴露在陽光下。';
  ...
  if (upper.includes('WL')) return '山泥傾瀉警告：遠離山坡及低窪地區，注意疏散指示。';
