...
function getDistrictFromCoords(lat, lng) {
	switch (true) {
		case lng >= 113.95 && lng <= 114.35 && lat >= 22.25 && lat <= 22.45:
			return "屯門區";
		case lng >= 113.85 && lng <= 114.25 && lat >= 22.35 && lat <= 22.55:
			return "元朗區";
		case lng >= 114.05 && lng <= 114.25 && lat >= 22.3 && lat <= 22.4:
			return "沙田區";
		case lng >= 114.15 && lng <= 114.25 && lat >= 22.3 && lat <= 22.35:
			return "大埔區";
		case lng >= 114.1 && lng <= 114.25 && lat >= 22.3 && lat <= 22.4:
			return "西貢區";
		case lng >= 114.1 && lng <= 114.2 && lat >= 22.25 && lat <= 22.35:
			return "葵青區";
		case lng >= 114.15 && lng <= 114.2 && lat >= 22.3 && lat <= 22.35:
			return "荃灣區";
		case lng >= 114.16 && lng <= 114.23 && lat >= 22.3 && lat <= 22.35:
			return "中西區";
		// ToDo: this is Duplicate by 荃灣區
...
function parseTideToday(json) {
	...
	for (let h = 1; h <= 24; h++) {
		...
		if (idx !== -1 && row[idx] !== undefined && row[idx] !== "") {
			hours.push({ hour: h, height: Number.parseFloat(row[idx]) });
		}
	}
	return hours;
}
